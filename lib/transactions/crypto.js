...
function getTransactionBytes ({
	type,
	asset: {
		signature, delegate, votes, multisignature,	dapp,
	...
	function isVoteTransaction () {
		const voteTransactionBytes = (Buffer.from(votes.join('')) || null);
	...
	function isMultisignatureTransaction () {
		...
		const LIFETIME = 1;
		const keysgroupBuffer = Buffer.from(multisignature.keysgroup.join(''), 'utf8');
	...
	function isDappTransaction () {
		let buf = new Buffer([]);
		const nameBuf = Buffer.from(dapp.name);
	...
	function isDappInTransferTransaction () {
		const buf = Buffer.from(inTransfer.dappId);
	...
	function isDappOutTransferTransaction () {
		const dappBuf = Buffer.from(outTransfer.dappId);
		const transactionBuf = Buffer.from(outTransfer.transactionId);
...
function createTransactionBuffer (transaction, options) {
	...
	const {assetSize, assetBytes} = getTransactionBytes(transaction);
